...
***************/
//        Vertex   1         2         3        RGB
	A->Triangle(0.1,0.6, -0.4,0.1 , 0.1,-0.4,  1,0,0);	// Draws the red
    A->Triangle(0.1,-0.4, 0.1,0.2, 0.4,-0.1,   1,1,0);  //Draws the yellow
    A->Triangle(-0.4,0.1, -0.4,0.8, 0.3,0.8,   0,0.5,1);  //Draws the light blue
    //Draw the purple head
	A->Triangle(0.1,-0.4, 0.5,-0.4, 0.3,-0.2,   0.5,0,0.5);
    A->Triangle(0.1,-0.4, 0.5,-0.4, 0.3,-0.6,   0.5,0,0.5);
    A->Triangle(0.1,-0.4, 0.3,-0.6, 0.1,-0.8,   1,0.5,1); //Draws the pink
    A->Triangle(0.5,-0.8, 0.5,-0.4, 0.3,-0.6,   0.1,0.8,0.9); //Draws the turquoise
    //Draw the green tail
    A->Triangle(-0.4,0.8, -0.7,0.8, -0.5,0.6,  0,1,0);
    A->Triangle(-0.5,0.6, -0.7,0.8, -0.8,0.6,  0,1,0);
}
